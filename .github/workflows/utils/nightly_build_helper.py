...
def add_next_versions(version, current_schedule):
    ...
    if version_obj.patch == 0:  # Handling minor version (e.g., 3.2.0)
        ...
        next_versions = get_next_versions(version)
...
def get_next_versions(version_str):
    """
    Given a semantic version, return list of next version(s) to create.
    For x.y.0 versions, returns [x.y.1, x.(y+1).0]
    For x.y.z versions (z>0), returns [x.y.(z+1)]
    """
    try:
        ...
        print(f"Error: Invalid semantic version '{version_str}': {e}")
        sys.exit(1)
